...
bool PIDSchema::Encode (const PIDConstants & pid_constants, size_t & bytes_written)
{
...
void PIDSchema::Decode (PIDConstants & pid_constants, std::size_t size) const
{
    ...
    {
        pid_constants.kp = p_pid.kp;
        pid_constants.ki = p_pid.ki;
        pid_constants.kd = p_pid.kd;
    }
}
...
void BrewProfileSchema::Decode (BrewProfile & brew_profile, std::size_t size)
{
    pb_istream_t stream = pb_istream_from_buffer (buffer, size);
    PBrewProfile p_brew_profile = PBrewProfile_init_zero;
