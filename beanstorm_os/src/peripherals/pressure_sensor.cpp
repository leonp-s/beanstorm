...
int I2CClearBus (int sdaPin, int sclPin)
{
    pinMode (sdaPin,
             INPUT_PULLUP); // Make sdaPin (data) and sclPin (clock) pins Inputs with pullup.
    pinMode (sclPin, INPUT_PULLUP);

    bool SCL_LOW = (digitalRead (sclPin) == LOW); // Check is sclPin is Low.
    if (SCL_LOW)
    { // If it is held low MCU cannot become the I2C master.
        pinMode (sdaPin,
                 INPUT_PULLUP); // Make sdaPin (data) and sclPin (clock) pins Inputs with pullup.
        pinMode (sclPin, INPUT_PULLUP);
    ...
    { //  vii. If sdaPin is Low,
        ...
        { // still low after 2 sec error
            pinMode (
                ...
                INPUT_PULLUP); // Make sdaPin (data) and sclPin (clock) pins Inputs with pullup.
            pinMode (sclPin, INPUT_PULLUP);
            return 2; // I2C bus error. Could not clear. sclPin clock line held low by slave clock
                      // stretch for >2sec
        }
    ...
    pinMode (sdaPin,
             OUTPUT); // and then make it LOW i.e. send an I2C Start or Repeated start control.
    // When there is only one I2C master a Start or Repeat Start has the same function as a Stop and
    // clears the bus.
...
void PressureSensor::I2CResetState ()
{
    if (digitalRead (SDA) != HIGH || digitalRead (SCL) != HIGH || ! ads_.isConnected ())
    {
        Serial.println ("Reset I2C pins");
        short result = I2CClearBus (SDA, SCL);
        char tmp [25];
...
void PressureSensor::GetAdsError ()
{
    I2CResetState ();

    short result = ads_.getError ();
    if (result == 0)
        return;
    char tmp [25];
